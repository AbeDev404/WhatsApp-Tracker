...
window.sleep = function(delay) { return new Promise(function(resolve) {setTimeout(resolve, delay)}) }
...
window.setTrackList = function (value) {
    window.trackList = JSON.parse(value)
}

window.getContacts = function () {
    window.ReactNativeWebView.postMessage(JSON.stringify({type: 'contacts', data: WAPI.getAllContacts()}));
}

setInterval(function() {
    if(document.getElementsByTagName('body')[0].innerHTML.indexOf('Link a device') >= 0) {
...
// const start = async () => {
//   var phone = ['+1 336-378-3836', '+1 423-408-8704', '+1 201-983-2089', '+1 843-999-5796'];
//   let index = 0;
//   while(true && window.tracking) {
//     var link = document.createElement("a");
//     link.setAttribute("href", "whatsapp://send?phone=" + phone[index]);
//     document.body.append(link);
//     link.click();
//     document.body.removeChild(link);
//     await sleep(5000)
//     const status = document.evaluate('/html/body/div/div/div/div[5]/div/header/div[2]/div[2]/span', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue
//     if(status !== null) console.log(status.innerHTML)
//     index++;
//     if(index === phone.length) index = 0;
//   }
// }
// start();